  ...
  return (
    <div >
      ...
      <div className={styles.mid_container}>
        {(jobType === 'all' || jobType === 'regjobs') && regjobs.map((job) => (
          <Link
            key={job.defJob_id}
        ...
        {(jobType === 'all' || jobType === 'freelancing') && freelancingjobs.map((job) => (
          <Link
            key={job.defJob_id}
            className={styles.job_card}
            to={`/job/${job.defJob_id}`}
...
const Offers = () => { const initialized = useRef(false);
    const filtered = useRef(false);
    const { accessToken } = useContext(LoginContext);
    const [isLoading, setIsLoading] = useState(false);
    const [regjobs, setRegJobs] = useState([]);
