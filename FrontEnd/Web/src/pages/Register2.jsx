        ...
        FetchSkills(event.target.value).then((response) => {
            if (response.status === 200) {
                ...
                response.data.skills.map((skill) => {
                    if (!checked[skill.id]) { setChecked((prevState) => ({ ...prevState, [skill.id]: false })) }
                })
            }
            else {
                console.log(response.statusText);
                    ...
                    <div className={styles.register__row}>
                        <div className={styles.title}>
                            ...
                            <div className={styles.scroll}>
                                {(types.length === 0) ? <><h1>GG</h1></> : types.map((type) => {
                                    return <button className={styles.available_skills} key={type.id} value={type.name} onClick={handleTypeSelect}>{type.name}</button>
                                })}
                            </div>
                        </div>
                        <div className={styles.title}>
                            ...
                            <div className={styles.scroll}>
                                {skills.map((skill) => (
                                    <button className={styles.available_skills} key={skill.id} value={skill.name} onClick={(event) => { handleChange(event, skill.id) }} disabled={checked[skill.id]}>{skill.name}</button>
                                ))
                                }
                            </div>
                        </div>
                        <div className={styles.title}>
                            ...
                            <div className={styles.choosedskills}>
                                {userSkills.map((skill) => (<button className={styles.skillbutton} key={skill.id} value={skill.name} onClick={(event) => { handleChange2(event, skill.id) }}>{skill.name}</button>))}
                            </div>
                        </div>
                    </div>
                </div>
